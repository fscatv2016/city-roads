...
<div class='find-place' :class='{centered: boxInTheMiddle }'>
  <div v-if='boxInTheMiddle'>
    <h3 class='site-header'>城市道路</h3>
    <p class='description'>渲染任意城市的每一条道路</p>
  </div>
  <form v-on:submit.prevent="onSubmit" class='search-box'>
      <input class='query-input' v-model='enteredInput' type='text' placeholder='输入城市名称点击开始' ref='input'>
  ...
  <div v-if='showWarning' class='prompt message note shadow'>
    注意：大城市可能需要200MB以上的数据传输，可能需要强大的设备来渲染。
  </div>
  <div class='results' v-if='!loading'>
    <div v-if='suggestionsLoaded && suggestions.length' class='suggestions shadow'>
      <div class='prompt message'>
        <div>选择下面的边界以下载其中的所有道路</div>
        <div class='note'>大城市可能需要200MB以上的数据传输和强大的设备</div>
      </div>
      <ul>
  ...
  <div v-if='error' class='error message shadow'>
    <div>很抱歉，我们无法从OpenStreetMap下载数据。目前，它可能正忙于处理其他请求。<br/><br/> 请将此网站添加书签，稍后<a href='#' @click.prevent="retry">重试</a>？</div>
    <div class='error-links'>
      <a href='https://twitter.com/anvaka/status/1218971717734789120' title='see what it supposed to do' target="_blank">看看它应该如何工作</a>
      <a :href='getBugReportURL(error)' :title='"report error: " + error' target='_blank'>报告此错误</a>
    </div>
  </div>
  <div v-if='loading' class='loading message shadow'>
    <loading-icon></loading-icon>
    <span>{{loading}}</span>
    <a href="#" @click.prevent='cancelRequest' class='cancel-request'>取消</a>
